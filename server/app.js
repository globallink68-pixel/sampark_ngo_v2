...
export function createApp(options={}){
  ...
  app.post('/api/contact',rateLimit({windowMs:60000,max:5}),async(request,response)=>{const details=contactInput(request.body);if(!details||request.body.website||!request.body.turnstile_token)return response.status(400).json({error:'Please check the enquiry details and consent.'});try{const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),5000),body=new URLSearchParams({secret:process.env.TURNSTILE_SECRET_KEY||'',response:request.body.turnstile_token}),verification=await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify',{method:'POST',body,signal:controller.signal});clearTimeout(timeout);const result=await verification.json();if(!verification.ok||result?.success!==true)throw Error('verification failed')}catch{return response.status(400).json({error:'Unable to verify your submission. Please try again.'})}await db().execute("insert into contact_messages(name,address,city,state,postal_code,email,mobile,subject,message,status) values(?,?,?,?,?,?,?,?,?,'new')",[details.name,details.address,details.city,details.state,details.postalCode,details.email,details.mobile,details.subject,details.message]);response.sendStatus(201)});
